...
import org.tyby.javaapacheexp.JavaApacheExp;
...
public class MainController  {
    ...
    private Button lastClickedButton = null; //上一次点击的按钮
    ...
    public void about() {
        ...
        Image image = new Image((getClass().getResource("/img/logo.jpg")).toString());
    ...
    public void initialize() {
        ...
        this.panda.setText(panda.getText().toUpperCase());


        this.panda.setOnAction((e) -> {
            try {
                //Desktop.getDesktop().browse(new URL("https://www.cnhonker.com/").toURI());
            } catch (Exception e1) {
                logger.debug(e1);
            }
        });
        ...
        for (Node node: selectButton.getChildren()){
            ...
            node1.setOnAction(event -> {
                // 恢复上一个被点击按钮的颜色
                if (lastClickedButton != null) {
                    lastClickedButton.setStyle("-fx-background-color: white;");
                }
                System.out.println(node1.getStyle());
                // 设置当前被点击按钮的颜色
                node1.setStyle("-fx-background-color: #007bff;");
                lastClickedButton = node1;

                refreshPage(node1.getAccessibleText());
            });
/*            if (node.getAccessibleText() != null){
    ...
    private void refreshPage(String page){
        try {
            ...
            //FXMLLoader(JavaApacheExp.class.getResource("/fxml/" + page + ".fxml"));
            FXMLLoader fxmlLoader = new FXMLLoader(JavaApacheExp.class.getResource("/fxml/" + page + ".fxml"));
            AnchorPane contentPage = fxmlLoader.load();
